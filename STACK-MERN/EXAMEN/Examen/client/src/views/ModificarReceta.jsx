...
const ModificarReceta = ({ me, recetas, setRecetas, logOut}) => {
    ...
    const {id}=useParams();
    const index = recetas.findIndex((receta)=> receta._id==id);
    ...
    const manejarEnvio = async (e) => {
        ...
        try {
          await axios.put(`http://localhost:8000/api/recetas/${id}`, 
            body, {headers:{token_user:localStorage.getItem("token")}})
            .then(
                response=>{
                            const copiaRecetas=[...recetas];
                            copiaRecetas[index]=response.data;
